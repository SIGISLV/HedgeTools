...
for strate in lnomstrate:
    arcpy.AddField_management(outfeature,strate,"TEXT")
...
for row in ucur:
    ...
    i=0
    for type in lsurface:
        if type>0:row.setValue(lnomstrate[i],1)
        else: row.setValue(lnomstrate[i],0)
        i+=1
    ucur.updateRow(row)
