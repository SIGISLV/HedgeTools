...
# on fait une copie de la classe d'entité en entrée.
...
lnomchamp=["buissonante", "arbustive", "arboree", "hautjet"]
for nomchamp in lnomchamp:
    arcpy.AddField_management(outfeature,nomchamp,"FLOAT")
...
# on fait une copie de la table polyhaid
...
for row in ucur:
    ...
    surfTot=reduce(add,lsurface)
    # on met à jour les champs de manière automatique.
    for nomchamp in lnomchamp:
        Prop=entsurf[nomchamp]/surfTot
        row.setValue(nomchamp,Prop)
        ucur.updateRow(row)