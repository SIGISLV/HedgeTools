...
arcpy.AddField_management(outfeature,"NbStrate","SHORT")
...
for row in ucur:
    ...
    # on cherche dans la liste toute les surfaces qui sont supérieure à 0. Dans le cas contraire il ne sera pas compter comme strate.
    nbstrate=0
    for surf in lsurface:
        if surf>0:nbstrate+=1
    row.setValue("NbStrate",nbstrate)
    ucur.updateRow(row)