...
# on fait une copie de l'emprise du polygones :
...
arcpy.AddField_management(outfeature,"StraCod","SHORT")
arcpy.AddField_management(outfeature,"StraDom","TEXT")
...
# il faut mettre à jour la table Polyhaid:
ucur=arcpy.UpdateCursor(outfeature)

lnomstrate=["buissonante","arbustive","arboree","hautjet"]
...
for row in ucur:
    ...
    # on récupère la position de la valeur maximale.
    stratedom=lsurface.index(max(lsurface))
    # on met à jour le champ Stradome.
    row.setValue("StraCod",stratedom+1)
    # on met à jour le champ Stratdom
    row.setValue("StraDom",lnomstrate[stratedom])
    ucur.updateRow(row)